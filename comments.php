<section id="comments">
    ...
    <?php if ( have_comments() ) : ?>
            <h2 id="comments-title">
                    <?php
                    printf( _n( 'New Comments  on &ldquo;%2$s&rdquo;', '%1$s Comments on &ldquo;%2$s&rdquo;', get_comments_number(), 'bp-magic' ),
                    number_format_i18n( get_comments_number() ), '<span class="commented-post-title">' . get_the_title() . '</span>' );
                    ?>
            </h2>
   ...
   <?php endif;  ?>
    ...
    <ol class="commentlist">
            <?php wp_list_comments( array( 'type'=>'comment','callback' => 'blogfused_comment_format' ) ); ?>
    </ol>
...
$req = get_option( 'require_name_email' );
	$aria_req = ( $req ? " aria-required='true'" : '' );
        ...
        $fields =  array(
		'author' => '<div class="comment-form-author input-prepend">' . '<label for="author"><span class="add-on"><i class="icon-user"></i></span>' . 
		            '<input id="author" name="author" type="text" value="' . esc_attr( $commenter['comment_author'] ) . '" size="20"' . $aria_req . ' placeholder="'.__('Your Name *','bp-magic').'"/>'.( $req ? '<span class="required"></span>' : '' ).'</label></div>',
		'email'  => '<div class="comment-form-email input-prepend"><label for="email"><span class="add-on"><i class="icon-envelope"></i></span>'  .
		            '<input id="email" name="email" type="text" value="' . esc_attr(  $commenter['comment_author_email'] ) . '" size="20"' . $aria_req . ' placeholder="'.__('Email *','bp-magic').'" />'.( $req ? '<span class="required"></span>' : '' ).'</label></div>',
		'url'    => '<div class="comment-form-url input-prepend"><label for="url"><span class="add-on"><i class="icon-globe"></i></span>'.
		            '<input id="url" name="url" type="text" value="' . esc_attr( $commenter['comment_author_url'] ) . '" size="20" placeholder="'.__('Website','bp-magic').'" /></label></div>',
	);
	...
	$comment_form_info = array(
		'fields'               =>  $fields ,
		'comment_field'        => '<p class="comment-form-comment"><label for="comment">' .'<textarea id="comment" class="span12" name="comment" cols="45" rows="8" aria-required="true" placeholder="'.__('Please Enter your comment here','bp-magic').'"></textarea></label></p>',
       ...
       blogfused_comment_form($comment_form_info); ?>
<?php endif; ?>
